...
export default class GlobalSheets extends Component {
    ...
    navigateToSheet(id) {
        ...
        Fetch.GET("QuestionSheet/GetGlobSheet/" + id)
            .then(x => x.json())
            .then(data => {
        ...
        return (
            ...
            >
                <div data-tip="Current folder and all things you can create in it." className="card-body">
                    ...
                    <div data-tip=""><NavLink to={c.createGlobalSheetPath + "/" + this.state.currentSheet.id}>Create Sheet</NavLink></div>
                    <div data-tip=""><NavLink to={c.createQuestionPath + "/" + this.state.currentSheet.id}>Create Question</NavLink></div>
                </div>
            </div>
        )
    }
                        ...
                        <div key={i} className="col-sm-4" onClick={() => this.onClickGlobalQuestion(x.id)}>
                            <div data-tip="" className="card mb-2" style={{ border: c.videoNotesBorder }}>
                                <div className="card-body">
                                    ...
                                    <a className="ml-1" onClick={(e) => this.onClickDeleteQuestion(e, x.id)} >Delete</a>
                                </div>
                            </div>
                        </div>
                    ))}
